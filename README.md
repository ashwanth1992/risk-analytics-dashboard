...
Early versions rebuilt everything from scratch on every run. Processing 400+ MB of raw files took 1–5 minutes, which made UI tweaks slow and frustrating. Splitting the pipeline into two stages — `engine_data.py` outputs a 23 MB JSON payload once; `engine_inject.py` injects it into the template in ~5 seconds — meant that CSS changes, label tweaks, or layout adjustments didn't require re-reading 400 MB of source data. This changed the development loop from minutes to seconds for UI work.
...
[Raw Data Sources]                         [~430 MB total]
  ...
  India state boundaries (GeoJSON, 3.8 MB) — polygon rendering
         │
         ▼
  engine_data.py          (~1–5 min)
  → dashboard_data.json   (23 MB, 17 serialized data structures)
         │
         ▼
  engine_inject.py + template_Finalized.html   (~5 sec)
  → Dashboard_Build/Phase3_Dashboard_Finalized.html   (47 MB, fully standalone)
```
...
# Open Dashboard_Build/Phase3_Dashboard_Finalized.html in any browser
```
...
```bash
python Verify_D1.py       # verify pincode and corridor volumes against raw CSV
